...
- 配色遵循 Git 双栏习惯：增加为绿、删除为红、修改在左侧显示旧值红色且右侧
  ...
  行分类和冲突判断由 Go 后端决定。
- 双方首行同列同名为 `id`，或全表只有一个同名且以 `ID` 结尾的候选表头时，可靠的唯一主键记录默认按主键对齐；多个候选不得猜测，空白、重复等歧义记录只对自身保守使用物理行，不得使整表放弃对齐。右键 A/B/C…列标题可在产生撤销历史前设置或取消当前表主键列，也可切换按行号比较。左右独有记录必须保留在相邻共同主键之间的原始邻近位置，不得统一追加到视图末尾。
- 冲突行要求存在同一套解析出的主键列、同一对齐记录主键相同且其余实际数据列全部不同；
