...
### 2026-08-06 未签名应用首次打开说明

- 三语 README 与官网下载页不再把 SHA-256 校验插入普通用户的首次打开主流程。Windows 以
  “下载并双击 → 更多信息 → 仍要运行”三步说明 SmartScreen 提示；macOS 以“解压并移动到
  应用程序 → 尝试打开 → 隐私与安全性 → 仍要打开”四步说明 Gatekeeper 放行。
- 文案解释 Windows 未签名、macOS 未签名且未公证导致系统无法自动验证发布者，但不把警告
  描述为恶意软件误报，也不建议全局关闭 SmartScreen、Smart App Control 或 Gatekeeper。
  组织管理设备没有单应用放行入口时，引导用户联系管理员。
- SHA-256 保留为首次打开说明之后的独立可选章节；官网下载页默认收起命令区，README 保留
  PowerShell 与 `shasum` 示例。三语渲染回归约束原因、关键按钮、主流程先于可选校验，并继续
  保证 `SHA256SUMS.txt` 可直接访问。
- `scripts/sync-product-content.mjs --check`、官网 lint、16 条静态路由生产构建及 11/11 渲染测试
  均通过。真实浏览器本地预览先因 vinext 静态资源尾斜杠重定向无法加载样式，切换为静态服务器后又
  被浏览器工具的 URL 安全策略阻止继续访问，因此本轮没有把真实浏览器视觉检查记为通过。
- 已通过 `scripts/deploy-site-lightsail.ps1` 原子部署到正式域名。源站三语下载页检查为 3/3；Caddy
