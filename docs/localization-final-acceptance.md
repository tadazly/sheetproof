...
产物证据，没有本轮独立的真实交互证据，因此在对应矩阵中继续标记为“未由 Codex 独立验证”。
2026-08-05，维护者明确确认当前国际化验收可以标记完成并接受这些证据边界；这些项目不再
作为结项阻塞。本轮仍存在 race 环境阻塞、macOS 未实机、少量底层诊断和网站 `<img>` warning，
因此最终结论为 Conditional Pass，而不是无条件 Pass。
...
这些项目没有被改写为 Codex 已执行或自动判定通过。维护者于 2026-08-05 明确确认当前
国际化验收可以标记完成并接受上述证据边界，因此它们不再阻塞结项；报告仍保留原始矩阵，
以便后续需要时补充可复现的逐项实机证据。
...
上述验收修复范围已在提交 `5d6f687` 中提交并推送。维护者随后确认国际化验收结项；本报告以
Conditional Pass 记录结果，不将允许的已知限制或未由 Codex 独立执行的项目隐藏为已验证。
提交范围包括：
