...
- **本地 Git 仓库模式**：可以手动选择或拖入本地仓库；导入过程会显示加载与错误。左侧读取真实工作区，右侧读取已有 Git 引用，不会 checkout、fetch 或切换分支。
- **安全的本地保存**：主动检测其他程序对工作簿的修改：只读侧提示后自动重载，可编辑侧先询问是否载入磁盘最新版；保存仍经过重开校验和原子替换。
