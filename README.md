...
- **理解工作簿语义的对比**：按工作表、值、公式、显式空值、语义类型与工作表顺序比较；共享字符串与内联字符串统一视为文本，不把 OOXML 存储编码噪声当成业务差异。
- **双栏差异审阅**：使用同步滚动的窗口化双栏网格，按增加、删除、修改和冲突分类定位真实差异。
- **只看当前工作表需要核对的行**：摘要只显示当前工作表的行数；增加、删除、修改、冲突可多选，快捷键 1–4 切换分类、5 切换全部差异行与完整表格。每次启动默认显示完整数据，切换筛选会保留正在核对的原始行位置。
...
对比结果中的四类数字只统计当前选中的工作表。四类筛选默认全部不选并显示完整表格；选中后左右两侧只保留对应差异行，缺失侧仍显示配对空行。快捷键 `1`、`2`、`3`、`4` 依次切换新增、删除、修改、冲突，`5` 切换“全部差异行 / 完整表格”。切换或取消筛选会保留正在核对的原始行及其视口位置；筛选状态不会跨启动保留。整行新增只对冲突行开放；仅当 `id` 列的现有非空数据都是整数时提供自动/指定 ID，属性名等文本 ID 列则原样追加整行。
...
**为什么看起来相同的单元格仍可能被标记？**  相等语义包含单元格是否存在、原始值、公式和语义类型；数字与文本、公式等真实类型差异会在底部显示。XLSX 共享字符串与内联字符串只是两种文本存储方式，不会单独产生差异。
