...
> 逐格看清 XLSX 差异，再把需要的改动合进来。

SheetProof 用左右对照方式检查 .xlsx 的值、公式和类型，支持直接比较两个文件，也支持 Git 仓库与 UGit 工作流。文件始终留在本机。 SheetProof 在本机读取工作簿，不依赖 Excel；桌面界面和 CLI 复用同一套 Go 差异、合并、撤销与安全保存核心。
...
- **可控合并与撤销**：只把确认过的单元格或整行写入左侧。编辑和合并都会进入撤销记录，保存前可以随时回退。
- **按住就能回看修改前**：按住左侧的“前后对比”，或在表格内按住 Tab，可以临时查看刚打开文件时的内容；松开后回到当前结果。
- **本地 Git 仓库模式**：左侧读取真实工作区，右侧读取已有 Git 引用。比较过程不会 checkout、fetch 或切换分支。
- **安全的本地保存**：保存前检查文件是否被其他程序改动，再通过同目录临时文件、重开校验和原子替换写回。

此外，应用支持双文件直接对比、Git/UGit 只读差异入口、单元格与整行合并、冲突行追加、会话撤销、JSON/text CLI 报告，以及外部修改检测。

## 适用场景

- **审阅配置表变更**：在提交或发版前核对数值、公式、类型和整行变化。
- **处理 Git 中的 XLSX 变化**：比较工作区与本地分支或远端跟踪引用，不打断当前分支上的工作。
- **选择性合并修正**：只接收确认过的单元格或整行，保存前仍可撤销。

## 下载与发布

当前版本是 **0.1.0 预览版**。目前尚未发布可执行文件，可以从源码构建。

- [查看 GitHub Releases](https://github.com/tadazly/ugxlsx/releases)：正式发布后可在这里下载安装包和校验文件。
